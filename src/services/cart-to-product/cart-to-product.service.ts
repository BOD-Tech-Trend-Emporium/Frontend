...
import { DeleteCartToProductByProductIdResponseDto } from '@entities/Cart.entity';
...
export class CartToProductService {
  ...
  public removeAProductFromCart(id: string): Observable<DeleteCartToProductByProductIdResponseDto>{
    const reqHeader = new HttpHeaders({
      'Authorization': `Bearer ${this.userData?.token}`
    });
    return this.httpClient.delete<DeleteCartToProductByProductIdResponseDto>(
